...
export default (canvas: HTMLCanvasElement, tools: HTMLElement, platform: PlatformId) => {
  ...
  game.run().then(() => {
    mountUi(tools, (ui) => {
      ui.group('bloom', () => {
        ui.bool(game.bloom, 'enabled')
        ui.scalar(game.bloom, 'threshold', { range: true, min: 0, max: 2 })
        ui.scalar(game.bloom, 'knee', { range: true, min: 0, max: 2 })
        ui.scalar(game.bloom, 'intensity', { range: true, min: 0, max: 10 })
        ui.scalar(game.bloom, 'steps', { range: true, min: 0, max: 10 })
      })
      ui.group('pixelate', () => {
        ui.bool(game.pixelate, 'enabled')
        ui.scalar(game.pixelate, 'size', { range: true, min: 1, max: 50 })
        ui.scalar(game.pixelate, 'gap', { range: true, min: 0, max: 1, step: 0.001 })
        ui.scalar(game.pixelate, 'corner', { range: true, min: 0, max: 1, step: 0.001 })
      })
    })
  })
  return () => {
...
class PongGame extends EcsGame {
  public width = 63
  public height = 27
...
class PaddleComponent implements GameComponent, InitializableComponent, BehaviorComponent {
  ...
  public updateBehavior(time: number, dt: number): void {
    ...
    if (this.game.state === 'running') {
      const upKey = this.isLeft ? KeyboardKeys.KeyW : KeyboardKeys.KeyI
      const downKey = this.isLeft ? KeyboardKeys.KeyS : KeyboardKeys.KeyK
      ...
      let axisValue = this.pads.axesValue(player, GamepadAxes.LeftVertical)
