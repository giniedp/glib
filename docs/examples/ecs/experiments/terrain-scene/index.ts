...
import { AutoMaterial, TerrainMaterial } from '@gglib/materials'
  ...
  public onInit() {
    ...
    this.content.loadBatch({
        ...
        TweakUi.mount('#tweak-ui', (ui) => {
          ui.group('Controls', () => {
            ui.group('Fog', () => {
              ui.color(material, 'FogColor', { format: '[n]rgb' })
              ui.slider(material, 'FogStart', { min: 0, max: 1000, step: 0.5 })
              ui.slider(material, 'FogEnd', { min: 0, max: 1000, step: 0.5 })
              ui.slider(material, 'FogDensity', { min: 0, max: 0.1, step: 0.0001 })
              ui.select(material, 'FogType', {
                options: [
                  { label: 'Off', value: 0 },
                  { label: 'Exp', value: 1 },
                  { label: 'Exp2', value: 2 },
            ...
            ui.group('Terrain', () => {
              ui.slider(material, 'Tiling', { min: 1, max: 128, step: 1 })
              ui.slider(material, 'Brightness', { min: 0.1, max: 2, step: 0.01 })
              ui.slider(material, 'Saturation', { min: 0.1, max: 2, step: 0.01 })
              ui.slider(material, 'Perturbation', { min: 0.1, max: 2, step: 0.01 })
              ui.slider(material, 'SlopeStrength', { min: 0.1, max: 1, step: 0.01 })
            })
          })
        })
