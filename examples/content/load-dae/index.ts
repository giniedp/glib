...
// # DAE Models
//
// Glib is able to load DAE models.
//
// ---
...
const baseUrl = '/assets/models/dae/'
const models = {
  Cube:          `${baseUrl}cube.dae`,
  'Dae Test':    `${baseUrl}dae-test.dae`,
  Tree:          `${baseUrl}tree.dae`,
}
...
TweakUi.build('#tweak-ui', (b) => {
  loadModel(models.Cube)
  b.select({ model: models.Cube }, 'model', {
    options: models,
    onChange: (ctrl) => loadModel(ctrl.value),
  })
  b.slider(controls, 'distance', { min: 0.01, max: 3})
  b.slider(controls, 'offset', { min: -3, max: 3 })
  b.slider(controls, 'fov', { min: 1, max: 120, step: 1 })
  b.checkbox(controls, 'update')
})
