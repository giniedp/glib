...
export const VEGETATION_SHADER = /* wgsl */ `
...
fn fs_main(input: VertexOutput) -> @location(0) vec4<f32> {
  ...
  let transmittance = saturate(dot(-surface.Normal.xyz, env.sunDirection) * 0.6 + 0.4);
  let backDiffuseMultiplier = 1.0;
  let backLight     = transmittance * env.sunColor * opacity * backDiffuseMultiplier;
