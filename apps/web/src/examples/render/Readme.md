...
The render package provides a `Renderer` class which when instantiatet
is responsible for managing render target textures and scenes.
An instance of the `Renderer` must be setup with a list of `RenderStage`s
where each stage is responsible for one particular part of the rendering pipeline.
Also the `Renderer` instance must be given one or more scenes to work with.
...
The following examples show the usage of the `Renderer` and how
