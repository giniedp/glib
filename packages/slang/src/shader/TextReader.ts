...
const TypeWhiteSpace = 0
const TypeSymbol = 1
type TextCharKind = 0 | 1
...
export class TextReader {
   ...
   */
  public get isSymbol() {
    return this.charMap.get(this.char) === TypeSymbol
  }
  ...
  public get isWhitespace() {
    return this.charMap.get(this.char) === TypeWhiteSpace
  }
   ...
   */
  constructor(text: string, options?: TextReaderOptions) {
    this.text = text
    this.options = options
    const whitespaces = options?.whitespaces ?? ' \t\r\n\f'
    whitespaces.split('').forEach((it) => {
      this.charMap.set(it, TypeWhiteSpace)
    })
    const symbols = options?.symbols ?? '.+-/*%<>[](){}^|&~=!:;,?'
    symbols.split('').forEach((it) => {
      this.charMap.set(it, TypeSymbol)
    })
  }
  ...
  public unread(count: number) {
    this.index = Math.max(0, this.index - Math.abs(count))
  }
