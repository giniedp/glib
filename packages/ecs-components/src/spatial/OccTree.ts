...
export class OccTree<T = any> implements SpatialSystem<T>, SpatialNode<T> {
  ...
  public static create<T = any>(min: IVec3, max: IVec3, maxDepth: number) {
    return new OccTree<T>(min, max, 0).subdivide(maxDepth)
  }
  ...
  public insert(entry: SpatialEntry<T>): boolean {
    ...
    if (index < 0) {
      (this.entries as Array<SpatialEntry<T>>).push(entry)
      return true
    }
    return false
  }
  ...
  public remove(entry: SpatialEntry<T>): boolean {
    const index = this.entries.indexOf(entry)
    if (index >= 0) {
      (this.entries as Array<SpatialEntry<T>>).splice(index, 1)
      return true
    }
    return false
  }
  ...
  public subdivide(maxDepth: number): this {
    ...
    if (!this.children) {
      ...
      for (let i = 0; i < 8; i++) {
        ...
        offset.y = i & 2 ? step.y : 0
        offset.z = i & 4 ? step.z : 0
   ...
   */
  public fit(volume: BoundingVolume): OccTree<T> {
  ...
  private testUp(volume: BoundingVolume): OccTree<T> {
    if (this.parent) {
  ...
  private testDown(volume: BoundingVolume): OccTree<T> {
    if (this.children) {
      for (let i = 0; i < this.children.length; i++) {
