 ...
 */
export class BoundingVolumeComponent implements GameComponent {
  /**
   * The spatial system where this volume will be inserted for intersection and visibility tests.
   */
  public spatial: SpatialSystemComponent
   ...
   */
  public volume: BoundingVolume & { update: (m: Mat4) => void }
  ...
  public initialize(entity: GameEntity<TransformComponent>): void {
    ...
    this.spatial = entity.provider.get(SpatialSystemComponent)
  }

  public activate(): void {
    this.loop.onUpdate.add(this.update)
    this.entity.transform.onUpdated.add(this.onTransformUpdated)
  }

  public deactivate(): void {
    this.loop.onUpdate.remove(this.update)
    this.entity.transform.onUpdated.remove(this.onTransformUpdated)
   ...
   */
  public update = () => {
    if ((this.volumeChanged || this.transformChanged) && this.entity.transform) {
      if (this.volume) {
        this.volumeChanged = false
        this.transformChanged = false
        this.volume.update(this.entity.transform.world)
        if (this.spatial) {
  ...
  public linkVolume(volume: BoundingVolume) {
    ...
    if (volume instanceof BoundingSphere) {
      this.volume = new UpdatableBoundingSphere(volume)
    } else if (volume instanceof BoundingBox) {
      this.volume = new UpdatableBoundingBox(volume)
    } else if (volume) {
