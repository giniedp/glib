...
import { BlendState, Color, Device, Texture } from '@gglib/graphics'
...
export class ParticleSystemComponent extends GameSystem {
  ...
  public emit(position: IVec3, velocity: IVec3, channel: string) {
    ...
    const random = Color.packToRGBA(Color.fromBytes(Math.random(), Math.random(), Math.random(), Math.random()))
    for (let i = 0; i < 4; i++) {
