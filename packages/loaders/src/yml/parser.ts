...
function read(state: { lines: string[]; index: number }, indent: string) {
  let result = {}
  let block: Node = null
  ...
  while (state.index < state.lines.length) {
    ...
    if (match) {
      ...
      if (lineIndent < indent) {
        ...
        // go one line up, it needs to be re-processed
        state.index -= 1
        block.object = read(state, lineIndent)
      } else {
...
/**
 * Parses a yaml formatted string and returns an object.
 *
 * @remarks
 * Supports only a very limited subset of YAML.
 * Basically it reads the yaml tree and collects the values. Values are kept as strings. There is no
 * conversion to numbers, booleans or any other data type. Multi line string values are supported.
