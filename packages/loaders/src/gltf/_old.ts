import { AsyncTask, ContentLoader, imageFromBlob, LoaderContext } from '@gglib/content'
import {
  BufferOptions,
  countBytes,
  DataType,
...
import { append, extname } from '@gglib/utils'
import {
  Accessor,
...
export type MaterialExtHandler = (loader: Loader, input: Material, output: MaterialOptions) => Promise<void>
export class Loader {
  ...
  /**
   * The url where this asset is loaded from
   */
  public url: string
  ...
  public async load(url: string, context: LoaderContext) {
    this.url = url
    ...
    const result = {
      ...
      materials: await Promise.all(this.document.materials.map((_, i) => this.loadMaterial(i))),
    }
    ...
    if (this.document.meshes) {
      tasks.push(
        ...this.document.meshes.map(async (_, index) => {
          result.meshes[index] = await this.createMesh(index, result.materials)
        }),
      )
    }

    if (this.document.animations) {
      for (let i = 0; i < this.document.animations.length; i++) {
        result.animations[i] = this.createAnimation(i)
      }
    }
    ...
    if (this.document.skins) {
      tasks.push(
        ...this.document.skins.map(async (_, index) => {
          result.skins[index] = await this.createSkin(index)
        }),
      )
    }
  ...
  protected loadDocument(url: string, context: LoaderContext) {
    return this.content.executor.run(async () => {
  ...
  protected async loadBuffers() {
    ...
    const tasks = (this.document.buffers || []).map((buffer, index): AsyncTask<void> => {
      return async () => {
        // embedded buffer
        if (!buffer.uri && this.document.chunks?.[index]) {
          buffers[index] = this.document.chunks[index]
          return
        }

        // external buffer - fetch with executor
        const url = this.content.resolveUrl(buffer.uri, this.url, this.context.baseUrl)
        const response = await this.content.fetch(url, {
          responseType: 'arraybuffer',
        })
        buffers[index] = response.body
      }
    })
    await this.content.executor.runAll(tasks, this.context.signal)
    return buffers
  }
  ...
  public getBufferView(index: number) {
    ...
    return {
      ...bufferView,
      buffer: this.getBuffer(bufferView.buffer),
    }
  }

  public getAccessor(index: number): GLTFAccessorBase {
    ...
    if (accessor.bufferView >= 0) {
      const view = this.getBufferView(accessor.bufferView)
      return new GLTFBufferViewAccessor(accessor, view.buffer, view)
    }

    if (accessor.sparse) {
      const sparse = accessor.sparse
      const iView = this.getBufferView(sparse.indices.bufferView)
  ...
  public async loadImage(index: number): Promise<TextureOptions> {
    return this.cached(`image-${index}`, async () => {
      ...
      // if (image.uri) {
      //   const url = this.content.resolveUrl(image.uri, this.url, this.context.baseUrl)
      //   return this.content.loadAsset(url, this.context).then((asset) => asset.textures[0])
      // }

      if (typeof image.bufferView === 'number') {
        const view = this.getBufferView(image.bufferView)
  ...
  public async loadTexture(index: number): Promise<TextureOptions> {
    ...
    return {
      ...imageOptions,
      generateMipmap: !surfaceFormatIsCompressed(imageOptions.format),
      name: texture.name,
      // TODO:
      // meta: texture.extras,
  ...
  public createAnimation(index: number): AnimationData {
    const srcAnimation = this.document.animations?.[index]

    if (!srcAnimation) {
      throw new Error(`[glTF] animation not found: ${index}`)
    }
    ...
    for (const srcChannel of srcAnimation.channels) {
      ...
      const accIn = this.getAccessor(srcSampler.input)
      const accOut = this.getAccessor(srcSampler.output)
  ...
  public async loadMaterial(index: number): Promise<MaterialOptions> {
    const input = this.document.materials?.[index] || {}
    const output: MaterialOptions = {
      name: input.name,
      meta: input.extras,
      properties: {},
    }

    const params = output.properties
    const tasks: Array<AsyncTask<void>> = []

    if (input.normalTexture != null) {
      tasks.push(() => {
        return this.loadTexture(input.normalTexture.index).then((texture) => {
          params.NormalMap = texture
        })
      })
      readTextureInfo(params, 'NormalMap', input.normalTexture)
    }
    if (input.occlusionTexture != null) {
      tasks.push(() => {
        return this.loadTexture(input.occlusionTexture.index).then((texture) => {
          params.OcclusionMap = texture
        })
      })
      readTextureInfo(params, 'OcclusionMap', input.occlusionTexture)
    }
    if (input.emissiveTexture != null) {
      tasks.push(() => {
        return this.loadTexture(input.emissiveTexture.index).then((texture) => {
          params.EmissiveColorMap = texture
        })
      })
      readTextureInfo(params, 'EmissiveColorMap', input.emissiveTexture)
    }
    if (input.emissiveFactor != null) {
      params.EmissiveColor = input.emissiveFactor
    }
    ...
    if (input.pbrMetallicRoughness) {
      // result.technique = 'pbr'

      const pbr = input.pbrMetallicRoughness
      ...
      if (pbr.metallicRoughnessTexture != null) {
        tasks.push(() => {
          return this.loadTexture(pbr.metallicRoughnessTexture.index).then((texture) => {
    ...
    const extPbrSpecGloss = getKhrExtension(input, 'KHR_materials_pbrSpecularGlossiness')
    if (extPbrSpecGloss) {
      ...
      if (ext.specularGlossinessTexture) {
        tasks.push(() => {
          return this.loadTexture(ext.specularGlossinessTexture.index).then((texture) => {
        ...
        tasks.push(() => {
          return this.loadTexture(ext.specularGlossinessTexture.index).then((texture) => {
            output.meta.SmoothnessMapChannel = 'a'
    ...
    const extSpecular = getKhrExtension(input, 'KHR_materials_specular')
    if (extSpecular) {
      params.SpecularColor = extSpecular.specularColorFactor || params.SpecularColor || [1, 1, 1]
      if (extSpecular.specularColorTexture) {
        tasks.push(() => {
          return this.loadTexture(extSpecular.specularColorTexture.index).then((texture) => {
            params.SpecularColorMap = texture
          })
        })
      ...
      if (extSpecular.specularTexture) {
        tasks.push(() => {
          return this.loadTexture(extSpecular.specularTexture.index).then((texture) => {
            output.meta.SmoothnessMapChannel = 'a'
            params.SmoothnessMap = texture
          })
        })
    ...
    const extIor = getKhrExtension(input, 'KHR_materials_ior')
    if (extIor) {
      params.IndexOfRefraction ??= extIor.ior ?? 1.5
    }

    const extNw = getNwExtension(input)
    if (extNw) {
      params.Metallic = 0
      if (extNw.smoothTexture) {
        tasks.push(() => {
          return this.loadTexture(extNw.smoothTexture.index).then((texture) => {
            params.SmoothnessMap = texture
          })
        })
    ...
    output.meta.alphaMode = input.alphaMode
    switch (input.alphaMode) {
      case 'BLEND':
        // params.Blend = true
        params.Alpha = 1.0
        break
      case 'MASK':
        // params.Blend = false
        if (typeof input.alphaCutoff === 'number') {
          params.AlphaClip = Math.max(0, input.alphaCutoff)
        } else {
    ...
    await this.content.executor.runAll(tasks, this.signal)

    return output
  }

  public createSkin(index: number): SkinData {
    ...
    if (!skin.inverseBindMatrices) {
      ...
      const accessor = this.getAccessor(skin.inverseBindMatrices)
  ...
  public createIndexBuffer(part: MeshPrimitive): BufferOptions {
    if (part.indices == null) {
      return null
    }

    const acc = this.getAccessor(part.indices)
  ...
  public createVertexBuffer(part: MeshPrimitive): VertexBufferOptions {
  ...
  public createGeometries(mesh: Mesh): GeometryOptions[] {
    const doc = this.document
    const result = mesh.primitives.map((part): GeometryOptions => {
      let min = [0, 0, 0]
      let max = [0, 0, 0]
      ...
      const iBufferOptions = this.createIndexBuffer(part)
      const vBufferOptions = this.createVertexBuffer(part)
  ...
  public createMesh(index: number, mats: MaterialOptions[]): MeshOptions {
    ...
    const parts = this.createGeometries(mesh)
    ...
    for (const part of parts) {
      if (!mtlMap.has(part.materialId)) {
        ...
        materials.push(mats[part.materialId])
      }
