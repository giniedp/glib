 ...
 * called the weight.
 */
export type Vertex = [number, number, number, number?] | [number, number, number, number, number, number]
...
export function parse(input: string): Document {
  ...
  eachLine(input, (line) => {
    ...
    switch (key) {
      ...
      case 'hole':
      case 'scrv':
        appendIfExists<BodyStatement<string, Curve2DReference[]>>(
          body,
          {
            type: key,
...
function readRefArray(
  item: string,
  minLength: 2,
): [VertexTextureNormalRef, VertexTextureNormalRef, ...VertexTextureNormalRef[]]
