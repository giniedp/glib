...
export const MTL_SPLATTING = Object.freeze({
  uniforms: glsl`
    #ifdef SPLATTING
    // @binding  splatMap
    // @filter   LinearWrap
  ...
  fs_surface: glsl`
    ...
    vec4 splatWeight = texture2D(textureSplat, vTexture.xy).rgba;
    float splatSlope = blendSlope(1.0 - vWorldNormal.y, splatUV);
    surface.Diffuse.rgb = splatColor(splatUV, splatWeight, splatSlope).rgb;
    #ifdef SPLATTING_TINT
    surface.Diffuse.rgb *= texture2D(textureTint, vTexture.xy).rgb;
    #endif
    #ifdef SPLATTING_NORMAL
    #ifdef V_TBN
    surface.Normal.xyz = normalize(vTBN * splatNormal(splatUV, splatWeight, splatSlope).rgb);
    #endif
    #endif
    #endif
  `,
