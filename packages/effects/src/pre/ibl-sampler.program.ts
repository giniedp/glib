...
//     uniform int uDistribution; // enum

//     // @binding isGeneratingLUT
//     uniform int uIsGeneratingLUT;

//     // @binding floatTexture
//     uniform int uFloatTexture; // 0: byte, 1: float

//     // @binding intensityScale
...
//       return texture(uTexture, src).rgb;
//     }
...
//       vec2 xi = hammersley2d(sampleIndex, uSampleCount);
...
//       // float omegaS = 1.0 / (float(uSampleCount) * pdf);
//       // // Solid angle of texel
//       // // note: the factor of 4.0 * MATH_PI
//       // float omegaP = 4.0 * MATH_PI / (6.0 * float(uWidth) * float(uWidth));
...
//               vec3 lambertian = textureLod(uCubemap, H, lod).rgb* uIntensityScale;
...
//                       lod = uLodBias;
//                   }
//                   vec3 sampleColor = textureLod(uCubemap, L, lod).rgb* uIntensityScale;
...
//       return vec3(4.0 * A, 4.0 * B, 4.0 * 2.0 * MATH_PI * C) / float(uSampleCount);
//     }

//     vec3 iblSample() {
//       vec3 color = vec3(0);
//       if(uIsGeneratingLUT == 0)
//       {
//           vec2 newUV = texCoord ;

//           newUV = newUV*2.0-1.0;

//           vec3 scan = uvToXYZ(uCurrentFace, newUV);
...
//           out vec4 fragColor;
//           void main() {
//             fragColor = vec4(panoramaToCubeMap(uCurrentFace, texCoord), 1.0) ;
//           }
//         `,
//       },
//     },
