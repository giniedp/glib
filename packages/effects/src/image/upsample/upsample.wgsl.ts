...
export const UPSAMPLE_WGSL_FS = /* wgsl */ `
  ...
  fn upsamplePassthrough(tex: texture_2d<f32>, samp: sampler, uv: vec2f) -> vec4f {
    return textureSample(tex, samp, uv);
  }

  // Standard 3x3 tent filter upsample, widely used for bloom mip-chain composites
  fn upsampleTent3x3(tex: texture_2d<f32>, samp: sampler, uv: vec2f, texelSize: vec2f) -> vec4f {
    let d = texelSize.xyxy * vec4f(1.0, 1.0, -1.0, 0.0);

    var s: vec4f;
    s  = textureSample(tex, samp, uv - d.xy);
    s += textureSample(tex, samp, uv - d.wy) * 2.0;
    s += textureSample(tex, samp, uv - d.zy);

    s += textureSample(tex, samp, uv + d.zw) * 2.0;
    s += textureSample(tex, samp, uv       ) * 4.0;
    s += textureSample(tex, samp, uv + d.xw) * 2.0;
  ...
  fn upsampleKawase(tex: texture_2d<f32>, samp: sampler, uv: vec2f, texelSize: vec2f) -> vec4f {
    var sum: vec4f = vec4f(0.0);

    sum += textureSample(tex, samp, uv + vec2f(-1.0, 0.0) * texelSize) * 2.0;
    sum += textureSample(tex, samp, uv + vec2f( 1.0, 0.0) * texelSize) * 2.0;
    sum += textureSample(tex, samp, uv + vec2f( 0.0,-1.0) * texelSize) * 2.0;
    sum += textureSample(tex, samp, uv + vec2f( 0.0, 1.0) * texelSize) * 2.0;
  ...
  fn upsampleBicubic(tex: texture_2d<f32>, samp: sampler, uv: vec2f, texelSize: vec2f) -> vec4f {
    ...
    result += textureSample(tex, samp, vec2f(texPos0.x,  texPos0.y))  * w0.x  * w0.y;
    result += textureSample(tex, samp, vec2f(texPos12.x, texPos0.y))  * w12.x * w0.y;
    result += textureSample(tex, samp, vec2f(texPos3.x,  texPos0.y))  * w3.x  * w0.y;

    result += textureSample(tex, samp, vec2f(texPos0.x,  texPos12.y)) * w0.x  * w12.y;
    result += textureSample(tex, samp, vec2f(texPos12.x, texPos12.y)) * w12.x * w12.y;
    result += textureSample(tex, samp, vec2f(texPos3.x,  texPos12.y)) * w3.x  * w12.y;

    result += textureSample(tex, samp, vec2f(texPos0.x,  texPos3.y))  * w0.x  * w3.y;
    result += textureSample(tex, samp, vec2f(texPos12.x, texPos3.y))  * w12.x * w3.y;
    result += textureSample(tex, samp, vec2f(texPos3.x,  texPos3.y))  * w3.x  * w3.y;
  ...
  fn fs(in: FragmentInput) -> @location(0) vec4f {
    let color = params.weight * upsample(in.uv);
