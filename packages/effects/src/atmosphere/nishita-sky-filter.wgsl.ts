...
fn fs_main(in: FragmentInput) -> @location(0) vec4f {
  ...
  let mieColor      = textureSample(mieScatteringMap, mieScatteringSampler, in.uv).rgb;
  let rayleighColor = textureSample(rayleighScatteringMap, rayleighScatteringSampler, in.uv).rgb;
  ...
  var color = mieColor * partialMieConst * miePhase;
  color = color + rayleighColor * partialRayleighConst * rayleighPhase;
