...
export function serve() {
  ...
  watch(path.join(project.dist, '**', '*'), { delay: 500 }).on('change', browserSync.reload)
}
...
export function watchPage(end) {
  ...
  const watchAssets = watch(project.page.assets.filter((it) => it.startsWith('assets')), { delay: 500 }, assets)
