      ...
      JSON.stringify(
        {
          ...
          module: `./index.js`,
          typings: `./index.d.ts`,
          peerDependencies: project
            .pkgPeerDependencies(pkg)
            .reduce((result, peer) => {
...
function updateReadme(pkg: string) {
  ...
  return new Promise((resolve, reject) => {
    fs.writeFile(
      project.pkgSrcDir(pkg, 'Readme.md'),
      `
...
function updateApiExtractorJson(pkg: string) {
  ...
  const pathToDist = path.relative(
    project.pkgSrcDir(pkg),
    project.pkgDstDir(pkg),
  )
  return new Promise((resolve, reject) => {
    fs.writeFile(
      project.pkgSrcDir(pkg, 'api-extractor.json'),
      JSON.stringify(
        {
          $schema:
          ...
          mainEntryPointFilePath:
            '<projectFolder>/' + path.join(pathToDist, `index.d.ts`),
          compiler: {
          ...
          apiReport: {
            ...
            reportTempFolder:
              '<projectFolder>/' + path.join(pathToRoot, `temp`),
          },
          ...
          docModel: {
            ...
            apiJsonFilePath:
              '<projectFolder>/' +
              path.join(pathToRoot, 'api', `${pkg}.api.json`),
          },
...
export function update(cb) {
  return parallel(
    ...project.packages.map((pkg) =>
      namedTask(`update:${pkg}`, () => updatePackage(pkg)),
    ),
  )(cb)
}
